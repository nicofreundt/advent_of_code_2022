...
def count_range_subsets(range_pairs):
    return sum([set(range(pair[0]['min'], pair[0]['max']+1)).issubset(set(range(pair[1]['min'], pair[1]['max']+1))) or set(range(pair[0]['min'], pair[0]['max']+1)).issuperset(set(range(pair[1]['min'], pair[1]['max']+1))) for pair in range_pairs])
